...
from .utils import create_excel_template, process_excel_file, process_student_registrations
...
from core.utils import send_student_email, send_student_email_via_brevo_api
import sys
...
def update_complaint(request, complaint_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            from core.utils import send_status_notification
            send_status_notification(ticket)
            mail_sent = True  # Assume success for message display
...
def send_welcome_email(student_email, first_name, roll_number, password):
    """Send welcome email to new student with login details."""
    subject = "Welcome to Apollo University - Your Student Account"
    
    html_content = f"""
    <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;'>
        <div style='background: #4a90e2; color: white; padding: 20px; text-align: center;'>
            <h2>Welcome to Apollo University</h2>
        </div>
        <div style='padding: 20px; background: #f9f9f9;'>
            ...
            <div style='background: white; border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; margin: 20px 0;'>
                ...
                <p><strong>Login URL:</strong> <a href='https://apollouniversity.edu.in/login'>https://apollouniversity.edu.in/login</a></p>
            </div>
            
            <p><strong>Important:</strong> You will be required to change your password on first login.</p>
            
            <p style='text-align: center; margin: 30px 0;'>
                <a href='https://apollouniversity.edu.in/login' 
            ...
            <p>Best regards,<br>Apollo University Support Team</p>
        </div>
    ...
    # Send the email
    return send_student_email_via_brevo_api(
        subject=subject,
...
def create_student(request):
    """Create a single student account with email notification"""
    if request.method == 'POST':
                ...
                try:
                    ...
                    if user:
                        ...
                        if hasattr(user, 'profile'):
                            ...
                            profile.save()
                            print("[DEBUG] Sending welcome email to new student.")
                            ...
                            if email_sent:
                                messages.success(request, f'Student account created successfully! Welcome email sent to {email}')
                            else:
                                messages.warning(request, f'Student account created, but failed to send welcome email to {email}')
                            form = CreateStudentForm()
                            return render(request, 'dept_admin/create_student.html', {'form': form})
                    else:
...
def escalate_ticket(request, ticket_id):
    print(f"[DEBUG] escalate_ticket view called for ticket_id: {ticket_id}")
    ticket = get_object_or_404(Ticket, id=ticket_id, department=request.user.profile.department)
    print(f"[DEBUG] Found ticket: {ticket.ticket_id}, status: {ticket.status}, student: {ticket.student.email}")
    
    if request.method == 'POST':
        reason = request.POST.get('reason')
        print(f"[DEBUG] POST received for escalation. Reason: {reason!r}", flush=True)
        sys.stdout.flush()
        if not reason:
        ...
        try:
            print(f"[DEBUG] escalate_ticket view: calling ticket.escalate for ticket {ticket.ticket_id}")
            ticket.escalate(request.user, reason)
            print(f"[DEBUG] escalate_ticket view: escalation complete for ticket {ticket.ticket_id}")
            messages.success(request, "Ticket escalated and student notified via email.")
        except Exception as e:
            print(f"[ERROR] escalate_ticket view: escalation failed for ticket {ticket.ticket_id}: {e}")
            messages.error(request, f"Escalation failed: {e}")
        return redirect('dept_admin:dashboard')
